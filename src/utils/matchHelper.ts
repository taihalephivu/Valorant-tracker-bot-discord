...
interface MatchDisplayOptions {
  gameName: string;
  tag: string;
  matchData: any;
}

export function createMatchEmbed(options: MatchDisplayOptions): EmbedBuilder | null {
  ...
  const sortedRedPlayers = match.players.red?.sort((a: any, b: any) => (b.stats.score || 0) - (a.stats.score || 0));
  sortedRedPlayers?.forEach((p: any) => {
  ...
  let bluePlayers = '';
  // Sắp xếp theo score từ cao xuống thấp
  const sortedBluePlayers = match.players.blue?.sort((a: any, b: any) => (b.stats.score || 0) - (a.stats.score || 0));
  sortedBluePlayers?.forEach((p: any) => {
    const kda = `${p.stats.kills}/${p.stats.deaths}/${p.stats.assists}`;
    const rank = p.currenttier_patched || 'Unranked';
  ...
  const targetPlayer = match.players.all_players?.find((p: any) => 
    p.name.toLowerCase() === gameName.toLowerCase() && p.tag.toLowerCase() === tag.toLowerCase()
  );
